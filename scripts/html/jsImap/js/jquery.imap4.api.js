/*
*   Author: Alexx Roche, alexx@cpan.org
...
(function($) {
  ...
  var login_form = '<form action="" name="loginform" method="post"><table>\
<tr><td>Username:</td><td><input type="text" name="authen_username" value="" autofocus required /></td></tr> \
<tr><td>Pass Phrase:</td><td><input type="password" id="password" name="authen_password" value=""/></td></tr> \
<tr><td colspan="2" align="center"><input class="small blue button" type="submit" name="submit" value="Submit"/></td></tr> \
</table><input type="hidden" name="destination" value="main" /></form>';

  $.imap.api = function(username, password, server, callerSettings) {
    var settings = $.extend({
          debug: true, // if enabled, will print most errors into browser console
          useSandbox: true, // set it to true, if you're working on your code, revert to false for live deploy
          host: this.useSandbox ? 'localhost' : 'imap4.alexx.net',
          ...
          onNonAuth: function(connectUrl, container) { // called when no valid token is found
            $('<center>'+login_form+'</center>').prependTo(container);
          },
        ...
        readToken = function() {
          var rToken = getParamInQuery(window.location.hash, 'access_token'),
              ...
              // for now, token is valid for one day
              tokenLife = 86400,
        ...
        connectUrl = 'https://' + settings.host + '/connect?client_id=' + username +'&response_type=token&redirect_uri=' + settings.redirect,
        authorize = function() {
